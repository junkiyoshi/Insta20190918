...
Actor::Actor(vector<glm::vec3>& location_list, vector<vector<int>>& next_index_list, vector<int>& destination_list) {
	...
	vector<int> hex_list = { 0x06BEE1, 0x1768AC, 0x2541B2, 0x03256C, 0xEFEFEF };
	int color_index = ofRandom(hex_list.size());
	this->color.setHex(hex_list[color_index]);
}
...
void Actor::draw() {
	...
	ofDrawBox(this->location, 9);

	ofNoFill();
	ofSetColor(39);
	ofDrawBox(this->location, 10);
}